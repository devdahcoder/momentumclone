...
const WeatherDailyItem = ({ day, index }) => {
	const renderIcon = () => {
		let weatherIcon =
			hour > 6 && hour < 18 ? day?.Day?.Icon : day?.Night?.Icon;
	...
	return (
		<div
			...
			onClick={() => handleCurrentWeatherDropDown(day, index)}
			tabIndex={index}
			ref={elementRef}
			id={`${activeLink === index ? "selected" : ""}`}
			className="weather-forecast-item weather-forecast-day"
			data-day="Friday"
			data-test="forecast-day"
			title={day && day.Day.IconPhrase}
		>
			<div
				className="daily-weather-forecast-label"
				data-test="forecast-day-label"
			>
				{DateToDay(day)}
			</div>

			<span
				className="icon icon-weather daily-weather-forecast-icon"
				data-icon="H"
				title={day && day.Day.IconPhrase}
			>
				{renderIcon()}
			...
			<div className="daily-weather-forecast-day-details">
				<span
					...
					data-test="forecast-high"
				>
					{day.Temperature.Maximum.Value.toFixed()}°
				</span>

				<span className="weather-forecast-low">
					{day.Temperature.Minimum.Value.toFixed()}°
				</span>
			</div>
		</div>
	);
